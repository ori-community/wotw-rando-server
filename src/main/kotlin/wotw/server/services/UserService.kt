...
class UserService(private val server: WotwBackendServer) {
    fun generateMultiverseInfo(multiverse: Multiverse) = MultiverseInfoMessage(multiverse.id.value, multiverse.universes.map { generateUniverseInfo(it) }, multiverse.board != null, multiverse.spectators.map { generateUserInfo(it) })
    ...
    fun generateUserInfo(user: User) = UserInfo(user.id.value, user.name, user.avatarId, server.connections.playerMultiverseConnections[user.id.value]?.multiverseId)
}